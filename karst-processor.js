class KarstProcessor extends AudioWorkletProcessor {
    constructor() {
        ...
        this.port.onmessage = (e) => {
            if (e.data.type === 'init') {
                this._initWasm(e.data.wasmBuffer).catch(err => {
    ...
    async _initWasm(wasmBuffer) {
        ...
        const stubs = {
            ...
            emscripten_resize_heap: (size) => {
                const need = Math.ceil((size - memory.buffer.byteLength) / 65536);
        ...
        // Carve output buffers from stack before karst_init — stack grows downward
        // so these pointers remain valid for the lifetime of the worklet
